...
For example, to change the dataset and the image detector, you could do::
    ...
    python -m pbtrack.main dataset=posetrack18 detect_multiple=openpifpaf

If you want to change a single value inside a category, for example the bbox extension factor::

    python -m pbtrack.main detect_multiple=openpifpaf "detect_multiple.cfg.bbox.extension_factor=[0.05,0.05,0.05]"
...
or at the key-level::
    ...
    python -m pbtrack.main -m dataset=posetrack18,posetrack21 detect_multiple=openpifpaf,yolox track.cfg.max_dist=0.2,0.5,0.7
...
The configuration for pbtrack is located in the ``configs/`` directory, with the following structure::
