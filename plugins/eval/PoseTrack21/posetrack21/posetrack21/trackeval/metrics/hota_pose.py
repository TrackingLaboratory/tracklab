...
class HOTAeypoints(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        if data['num_tracker_dets'] == 0:
            res['HOTA_FN'] = data['num_gt_joints'][None, :] * np.ones((len(self.array_labels), self.n_joints), dtype=float)
        ...
        if data['num_gt_dets'] == 0:
            res['HOTA_FP'] = data['num_tracker_joints'][None, :] * np.ones((len(self.array_labels), self.n_joints), dtype=float)
            res['LocA'] = np.ones((len(self.array_labels), self.n_joints), dtype=float)
            res['LocA(0)'] = np.ones((self.n_joints), dtype=float)
            res = self._compute_final_fields(res, compute_avg=True)
            return res
