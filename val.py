...
@torch.no_grad()
def val(eval_cfg, dekr_cfg, strongsort_cfg, *args, **kwargs):
    ...
    if osp.exists(save_dir):
        i = 0
        while osp.exists(save_dir + str(i)):
            i += 1
        save_dir = save_dir + str(i)
        print(
            f"Save directory ({osp.join('runs', eval_cfg['save_dir'])}) already exists."
        )
    ...
    model_pose = DEKRDetector(
        dekr_cfg, device, vis_threshold=0.3  # FIXME add vis channel
    )
    ...
    for data in tqdm(dataset, desc="Inference"):  # tensor RGB (3, H, W)
        if data["frame"] == 1:  # new video
            ...
            model_track = StrongSORTTracker(strongsort_cfg, device)
    ...
    if eval_cfg["MOT"]["eval"]:
        ...
        eval_cfg["MOT"]["DATASET_CFG"]["GT_FOLDER"] = osp.join(
            eval_cfg["posetrack_dir"], "posetrack_mot", "mot", eval_cfg["subset"]
        )
    ...
    if eval_cfg["pose_estimation"]["eval"]:
        ...
        eval_cfg["pose_estimation"]["EVAL_CFG"]["LOG_ON_ERROR"] = osp.join(
            pose_estimation_dir, "error_log.txt"
        )
        eval_cfg["pose_estimation"]["DATASET_CFG"]["GT_FOLDER"] = osp.join(
            eval_cfg["posetrack_dir"], "posetrack_data", eval_cfg["subset"]
        )
        eval_cfg["pose_estimation"]["DATASET_CFG"][
            "TRACKERS_FOLDER"
        ] = pose_estimation_dir
        evaluator = trackeval.PoseEvaluator(eval_cfg["pose_estimation"]["EVAL_CFG"])
        dataset_list = [
            trackeval.datasets.PoseTrack(eval_cfg["pose_estimation"]["DATASET_CFG"])
        ]
        metrics_list = []
    ...
    if eval_cfg["pose_tracking"]["eval"]:
        eval_cfg["pose_tracking"]["EVAL_CFG"]["LOG_ON_ERROR"] = osp.join(
            pose_tracking_dir, "error_log_track.txt"
        )
        eval_cfg["pose_tracking"]["DATASET_CFG"]["GT_FOLDER"] = osp.join(
            eval_cfg["posetrack_dir"], "posetrack_data", eval_cfg["subset"]
        )
        ...
        dataset_list = [
            trackeval.datasets.PoseTrack(eval_cfg["pose_tracking"]["DATASET_CFG"])
        ]
        metrics_list = []
    ...
    if eval_cfg["reid_pose_tracking"]["eval"]:
        eval_cfg["reid_pose_tracking"]["EVAL_CFG"]["LOG_ON_ERROR"] = osp.join(
            pose_tracking_dir, "error_log_reid_track.txt"
        )
        eval_cfg["reid_pose_tracking"]["DATASET_CFG"]["GT_FOLDER"] = osp.join(
            eval_cfg["posetrack_dir"], "posetrack_data", eval_cfg["subset"]
        )
        eval_cfg["reid_pose_tracking"]["DATASET_CFG"][
            "TRACKERS_FOLDER"
        ] = pose_tracking_dir
        evaluator = trackeval.EvaluatorReid(eval_cfg["reid_pose_tracking"]["EVAL_CFG"])
        dataset_list = [
            trackeval.datasets.PoseTrackReID(
                eval_cfg["reid_pose_tracking"]["DATASET_CFG"]
            )
        ]
        metrics_list = []
