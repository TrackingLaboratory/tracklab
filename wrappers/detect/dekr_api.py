...
class DEKR(Detector):
    ...
    def _reshape_image(self, image):
        ...
        if image.shape[0] > image.shape[1]: # H > W
            ...
            dim = (int(ratio * image.shape[1]), self.cfg.DATASET.INPUT_SIZE) # new width, height
        else: # W > H
            ratio = float(self.cfg.DATASET.INPUT_SIZE) / float(image.shape[1])
            dim = (self.cfg.DATASET.INPUT_SIZE, int(ratio * image.shape[0])) # new width, height
    ...
    def process(self, preprocessed_batch, metadatas):
        ...
        detections = []
        for ((_, metadata), base_size, center, scale_resized, scales, image_resizeds) in \
            zip(metadatas.iterrows(), base_sizes, centers, scale_resizeds, scaless, image_resizedss):
                poses = self._process_images(image_resizeds, scales, base_size, center, scale_resized)
                for pose in poses:
                    detections.append(
                        Detection.create(
