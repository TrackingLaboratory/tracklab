...
# FIXME FILE WILL BE DELETED IN FUTURE
class PoseTrack21(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return dict(image=F.pil_to_tensor(image), metadata=metadata)  # type: ignore


class ImageFolder2(torch.utils.data.Dataset):
    def __init__(self, path):
        assert os.path.isdir(path), (
            f"{path} is not a directory. path should point to" + " a folder with images"
        )
