...
def bbx_match_score(warped_corr_pose, curr_bbx, valid_corr_pose_joints, corr_threshold, num_valid_corr_pose_joints):
    match_score = 0
    points_inside = 0
    ...
    for j in range(17):
        ...
        if curr_bbx[0] <= warped_corr_pose[0, j] <= curr_bbx[2] and curr_bbx[1] <= warped_corr_pose[1, j] <= curr_bbx[
            3]:
...
def correlation_score(curr_pose, correlations, joint_threshold, corr_threshold):
...
def compute_OKS(gt, dt, valid_joints, bb, area, joint_threshold):
...
def pose_match_score(curr_bbx, curr_pose, warped_corr_pose, corr_threshold, joint_threshold):
    ...
    if np.count_nonzero(valid_joints) > 0:
        OKS, joint_oks = compute_OKS(curr_pose.transpose([1, 0]), pose_det, valid_joints, curr_bbx, area,
                                     joint_threshold)
    else:
        OKS = 0
        joint_oks = 0
...
def perform_tracking(tracks, affinities, affinities_clean, votes, frame_data, features, joint_threshold, ann_ids,
                     min_keypoints, f, max_detections_per_image, duplicate_ratio, break_tracks,
                     images=None):
    ...
    for k in range(max_detections_per_image):
        ...
        if best_t >= 0 and best_corr > 0:
            ...
            track['queries'] = queries_u
            track['kpts'] = frame_kpts[k]
            track['ann_id'].append(ann_id)
            track['features'] = features[f]
            track['curr_frame'] = f
            track['num_kpts'] = np.count_nonzero(frame_kpts[k][2] >= joint_threshold)
            track['bbx'] = frame_bboxes[k]
...
def get_pose_from_correlations(correlations, frame_warps, res_h, res_w, corr_threshold):
...
def get_affinities(tracks, features, model, frame_data,
                   fr_idx,
                   break_tracks, joint_threshold, corr_threshold, oks_threshold,
                   res_h, res_w, min_keypoints, max_detections_per_image, images=None):
    vis = False
