...
def load_set(anns_path, dataset_path, split):
    ...
    return TrackingSet(split, Detections(detections), ImageMetadatas(image_metadatas), VideoMetadatas(video_metadatas))
...
def load_annotations(anns_path, dataset_path, split):
    ...
    detections['keypoints'] = detections['keypoints'].apply(lambda x: np.reshape(np.array(x), (-1, 3)))
    ...
    image_metadatas = image_metadatas.rename(columns={'vid_id': 'video_id',
                                            'file_name': 'file_path',
                                            'nframes': 'nframe'})
    ...
    return detections, image_metadatas, video_metadatas
