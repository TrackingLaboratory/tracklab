...
class SoccerNetMOTEvaluator(EvaluatorBase):  # TODO replace with generic MOT evaluator
    def __init__(self, cfg, *args, **kwargs):
        self.cfg = cfg

    def run(self, tracker_state):
        log.info("Starting evaluation on SoccerNet")
        image_metadatas = (
        ...
        if self.cfg.eval_mot:
            ...
            sequences = [seq for seq in ts]
            for seq in sequences:
                ts[seq].ClassId = ts[seq].Visibility = 1
                gt[seq][0].ClassId = gt[seq][0].Visibility = 1
